...
let concursoReal = () =>{
    ...
    for(let i = 0; ingreso === "s"; i++){
        ...
        menores.push((Number(minAnios)*360) + (Number(minMeses)*30) + (Number(minDias)))
        maximos.push((Number(maxAnios)*360) + (Number(maxMeses)*30) + (Number(maxDias)))
        ingreso = prompt("Desea agregar otro delito (s/n): ")
        while (ingreso != "n" && ingreso != "s"){
            console.log("Valor incorrecto, por favor ingrese s/n")
            ingreso = prompt("Desea agregar otro delito (s/n): ")
        }
    }
    minimoMaximo = Math.max(...menores);
    ...
    sumaMaximos = maximos.reduce((a,b) => a+b, 0)
    ...
    console.log(`la pena aplicando el concurso real es de ${minimoMaximo} a ${sumaMaximos}`)
    if (Math.max(...menores) > 1080){
        console.log("En caso de resultar condenado, la condena será de ejecución efectiva")
    } else{
        console.log("En caso de resultar condenado y no tener condena previa, podrá tener una pena de ejecución condicional")
    }
